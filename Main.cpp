...
void Item_respawn(Item& item, const Map map, Snake snake, int drug_dur) {
	// 위치가 겹치지 않게 아이템 생성
	int new_x, new_y;
	bool sc;
	do {
	...
	item.Item_type = (drug_dur == 0 && rn == 2) ? 2 : rn % 2;
	item.active = true;
}
...
int main() {
	...
	for (int stage = 1; ; ++stage) {
		...
		for (int i = 0; i < itemNum; i++) {
			Item_respawn(items[i], map, snake, drug_dur);
		}
		...
		while (true) {
			...
			if (tick % snakeTick == 0) {
				...
				for (int index = 0; index < itemNum; ++index) {
					...
					if (snake.getPositionOf(0)[0] == items[index].x &&
						...
						if (items[index].Item_type == 0) { //growthItem일 경우 길이 증가
							snake.increaseLength();
							growthCnt++;
						}
						else if (items[index].Item_type == 1) {//poisonItem일 경우 길이 감소
							snake.decreaseLength();
							poisonCnt++;
						}
						else {
							drug_dur = 30; // 마약 효과 지속시간을 틱 30으로 설정
						}


						Item_respawn(items[index], map, snake, drug_dur);
			...
			if (tick % itemTick == 0) {
				//아이템 재출현
				for (int i = 0; i < itemNum; i++)  Item_respawn(items[i], map, snake, drug_dur);
			}
